<?php if (isset(Route::$slug) && !empty($data['user'])): ?>
	<?php if ($data['user']['userId'] != $_SESSION['user_id']): ?>
		<div class="content">
			...
			<div class="content_portfolio">
				...
				<!-- МЕНЮ КОНТАКТА -->
				<?php if (in_array(Route::$slug,$_SESSION['contacts'])): ?>
					<!-- добавить в контакты -->
					<a href="/<?=Route::$page;?>/delete_contact/<?=$data['user']['userId'];?><?php if(isset($_GET['search'])) echo '/?search='.$_GET['search'];?>">delete contact</a>
					<!-- добавить в контакты -->
					<!-- удалить чат -->
					<?php if (!empty($data['chat'])): ?>
						<a href="/<?=Route::$page;?>/drop_chat_warning/<?=$data['user']['userId'];?><?php if(isset($_GET['search'])) echo '/?search='.$_GET['search'];?>">drop</a>
					<?php endif ?>
					<!-- удалить чат -->
					<!-- очистить чат -->
					<?php if (!empty($this->data['messages'])): ?>
						<a href="/<?=Route::$page;?>/clean_messages_warning/<?=$data['user']['userId'];?><?php if(isset($_GET['search'])) echo '/?search='.$_GET['search'];?>">clean</a>
					<?php endif ?>
					<!-- очистить чат -->

				<?php else: ?>
					<!-- ДОБАВИТЬ В КОНТАКТЫ -->
					<a href="/<?=Route::$page;?>/add_contact/<?=$data['user']['userId'];?><?php if(isset($_GET['search'])) echo "/?search=".$_GET['search'];?>" class="content-portfolio-add-contact">add contact</a>	
					<!-- ДОБАВИТЬ В КОНТАКТЫ -->
				<?php endif ?>
				<!-- МЕНЮ КОНТАКТА -->
